...
9. [x] **Убрать дублирующее WS-уведомление о статусе заказа**
...
10. [x] **Прогнать `flutter analyze` и `flutter test`** — чисто; единственный провал (`test/widget_test.dart`, шаблонный Counter-тест) пре-существующий и не связан с этим планом (проверено через `git stash`)
    Блокируется: 1–9.
